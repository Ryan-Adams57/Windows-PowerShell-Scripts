...
Version:        1.0
website:        https://github.com/Ryan-Adams57
...
# Confirmation for removing duplicate license assignment
...
Get-MgBetaUser -Filter "assignedLicenses/`$count ne 0" -All -Property $RequiredProperties -ConsistencyLevel eventual -CountVariable Records | select $RequiredProperties | ForEach-Object {
    ...
    $ExportResult = [PSCustomObject]@{
        ...
        'UPN' = $UPN
        'Directly Assigned Licenses' = $DirectlyAssignedLicense
        'Group Based Licenses' = $GroupBasedLicense
        'Duplicate License(s)' = $DuplicateAssignment
        'Duplicate License Count' = $DuplicateLicenseCount
        'Directly Assigned Licenses(Frndly_Name)' = $DirectlyAssignedLicense_FrndlyName
        'Group based Licenses(Frndly_Name)' = $GroupBasedLicense_FrndlyName
    ...
    # Remove Direct License Assignment if the same license assigned via group too
...
if((Test-Path -Path $ExportCSV) -eq "True")
{
    ...
    Write-Host "~~ Check out " -NoNewline -ForegroundColor Green; Write-Host "https://github.com/Ryan-Adams57" -ForegroundColor Yellow -NoNewline; Write-Host " for more scripts and tools. ~~" -ForegroundColor Green `n`n
