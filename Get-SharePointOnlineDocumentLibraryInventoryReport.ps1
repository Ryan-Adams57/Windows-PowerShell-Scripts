<#------------------------------------------------------------------------------------------------------------------------------------
Name: Get SharePoint Online Document Library Inventory Report Using PowerShell
Version: 1.0
...
function Get_Statistics
{
    ...
    Get-PnPList  | Where-Object {$_.BaseType -eq "DocumentLibrary" -and $_.Hidden -eq $false} | ForEach-Object{
        ...
        {
            $LibrarySize = Get-PnPFolderStorageMetric -List $_.Title | Select-Object TotalSize,TotalFileCount
            ...
            $LibrarySizeFormatted = Convert_ToNearestUnit -LibrarySizeInBytes $LibrarySizeInBytes
            $ExportResult = @{
                ...
                "Library Size" = $LibrarySizeFormatted
            }
...
if($SiteAddress -ne "")
{
    ...
    $Web = Get-PnPWeb | Select-Object Title,Url
...
elseif($SitesCsv -ne "")
{
    try
    {
        Import-Csv -Path $SitesCsv | ForEach-Object{
            ...
            $Web = Get-PnPWeb | Select-Object Url,Title 
            Get_Statistics -SiteUrl $Web.Url -SiteTitle $Web.Title
        }
    }
    catch
