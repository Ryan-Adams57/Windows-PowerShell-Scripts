<#
=============================================================================================
Name:           Export Distribution Groups a user is member of
...
{       
    ...
    # Storing credentials in script for scheduling purposes/ Passing credentials as parameter - Authentication using non-MFA account
...
{
    # Finding Distribution List that User is a Member of
    $Result= @()
    $DistinguishedName=$_.DistinguishedName
    $Filter = "Members -Like ""$DistinguishedName"""
    $UserPrincipalName=$_.UserPrincipalName
    $UserDisplayName=$_.DisplayName
    Write-Progress -Activity "Finding Distribution Lists that user is a member of" -Status "Processed User Count: $Global:ProcessedUserCount" -CurrentOperation "Currently Processing $UserPrincipalName"
    ...
    $Result=New-Object PsObject -Property @{'User Principal Name'=$UserPrincipalName;'User Display Name'=$UserDisplayName;'No of DLs that user is a member of'=$DLsCount;'DLs Name'=$DLsName -join ',';'DLs Email Address'=$DLsEmailAddress -join ',';} 
    $Result | Select-Object 'User Principal Name','User Display Name','No Of DLs That User Is A Member','DLs Name','DLs Email Address' | Export-Csv  $OutputCsv -NoTypeInformation -Append 
...
{          
    ...
    {           
        ...
        Write-Host "The Output file is available at:" -NoNewline -ForegroundColor Yellow; $OutputCsv
        Write-Host `n~~ Script prepared by Ryan Adams ~~`n -ForegroundColor Green
        Write-Host "~~ Check out " -NoNewline -ForegroundColor Green; Write-Host "GitHub - https://github.com/Ryan-Adams57" -ForegroundColor Yellow -NoNewline; Write-Host " for more scripts and resources. ~~" -ForegroundColor Green `n`n
...
{  
    Try
    {
        Write-Host "Checking if $UserPrincipalName is a valid user"
        Get-Mailbox -Identity $UserPrincipalName -ErrorAction Stop | ForEach{
            List_DLs_That_User_Is_A_Member
        }
    }
