<#
========================================================================================
Name:                    Invoke Compromised Microsoft Account Response Workflow
...
Function ConnectModules 
{
    ...
    try{
        ...
        {
            Connect-MgGraph -Scopes "Directory.AccessAsUser.All","user.ReadWrite.All","UserAuthenticationMethod.Read.All"  -ErrorAction SilentlyContinue -Errorvariable ConnectionError |Out-Null
            if($ConnectionError -ne $null)
            {
                Write-Host $ConnectionError -Foregroundcolor Red
                Exit
...
{ 
   ConnectModules
    ...
    $ExportCSV =  "$Location\CompromisedUser_Remediation_Status_$((Get-Date -format yyyy-MMM-dd-ddd` hh-mm-ss` tt).ToString()).csv"
    $PasswordLogFile = "$Location\PasswordLog_$((Get-Date -format yyyy-MMM-dd-ddd` hh-mm-ss` tt).ToString()).txt"
    $InvalidUserLogFile = "$Location\InvalidUsersLog_$((Get-Date -format yyyy-MMM-dd-ddd` hh-mm-ss` tt).ToString()).txt"
    $ErrorsLogFile = "$Location\ErrorsLog_$((Get-Date -format yyyy-MMM-dd-ddd` hh-mm-ss` tt).ToString()).txt"
    ...
    Foreach($UPN in $UPNs)
    {
        ...
        {
            switch($Action){
                1  { Write-Progress -Activity "Processing $UPN" -Status "Disabling user"; DisableUser ; break }
                2  { Write-Progress -Activity "Processing $UPN" -Status "Signing out from all the sessions"; SignOutFromAllSessions ; break }
                3  { Write-Progress -Activity "Processing $UPN" -Status "Resetting password"; ResetPasswordToRandom ; break }
                4  { Write-Progress -Activity "Processing $UPN" -Status "Retrieving registered authentication methods"; ReviewMFAMethods ; break }
                5  { Write-Progress -Activity "Processing $UPN" -Status "Disable inbox rules"; DisableInboxRule ; break }
                6  { Write-Progress -Activity "Processing $UPN" -Status "Retrieving SMTP email forwarding configuration set by user"; ReviewEmailForwardingConfiguredByUser ; break }
                7  { Write-Progress -Activity "Processing $UPN" -Status "Removing SMTP forwarding address"; RemoveEmailForwardingConfiguredByUser ; break }
                8  { Write-Progress -Activity "Processing $UPN" -Status "Exporting activity audit logs"; GetActivityLog ; break }
                Default {
        ...
        'Disable Inbox Rule'=$DisableInboxRuleAction;
        'SignOut From All Sessions'=$SignOutFromAllSessionsAction;
        'Registered MFA Methods'=$ReviewMFAMethodsAction;
        'Review Email Forwarding Set by User'=$ReviewEmailForwardingConfiguredByUserAction;
        'Remove Email Forwarding Set by User'=$RemoveEmailForwardingConfiguredByUserAction;
        'Export Audit Log'=$GetActivitytLogAction} 
    ...
    Write-Host "`nScript maintained by Ryan Adams"
    ...
    {
        Write-Host `nErrors log file available in -NoNewline -Foregroundcolor Yellow; Write-Host " $ErrorsLogFile .You can see the reason for failed actions." 
    }
