...
Description:    This script finds all inbox rules that forward emails externally in Office 365 using PowerShell
...
4. Helps to filter-out forwarding rules that forward emails to external users by excluding guest accounts. 
...
else
{
    ...
    else
    {
        ...
        {   
            Write-Host "The InboxRule with external email configuration report available in:" -NoNewline -ForegroundColor Yellow; Write-Host "$OutputCsv" 
            $Prompt = New-Object -ComObject wscript.shell    
            $UserInput = $Prompt.popup("Do you want to open
