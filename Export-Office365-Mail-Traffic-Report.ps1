...
Function Install_Modules
{
 ...
 { 
  ...
  { 
   Write-Host EXO V2 module is required to connect Exchange Online. Please install the module using Install-Module ExchangeOnlineManagement cmdlet. 
   Exit
  }
 }  
 ...
 if($OnlyOrganizationUsers.IsPresent)
 {
  ...
  { 
   ...
   { 
    Write-Host MSOnline module is required to generate the report. Please install the module using Install-Module MSOnline cmdlet. 
    Exit
   }
  }
 }
...
if((Test-Path -Path $OutputCSV) -eq "True") 
 {
  ...
  Write-Host "~~ Check out " -NoNewline -ForegroundColor Green; Write-Host "https://www.governmentcontrol.net/" -ForegroundColor Yellow -NoNewline; Write-Host " for additional resources. ~~" -ForegroundColor Green `n`n
