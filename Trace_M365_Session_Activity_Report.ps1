...
Script Highlights:
1. Retrieves all activities performed during a specific session. By default, it can access up to 180 days of data. You can customize the date range using the StartDate and EndDate parameters. 
2. Generates a well-organized, user-friendly CSV report for immediate use. 
3. Supports execution via Certificate-Based Authentication (CBA). 
...
Write-Host `nRetrieving sessionId-based activity log from $StartDate to $EndDate... -ForegroundColor Yellow
$i=0
$ExportResult=""   
$ExportResults=@()  
...
if($SessionId -eq "")
{ 
 $SessionId=Read-Host Enter session Id '(eg:007a6809-fby1-b3ee-799f-5bafb8ab1740)'
}
...
{ 
 ...
 $Results=Search-UnifiedAuditLog -StartDate $CurrentStart -EndDate $CurrentEnd -UserIds $UserID -FreeText $SessionId -SessionId s -SessionCommand ReturnLargeSet -ResultSize 5000
...
Write-Host "~~ Check out " -NoNewline -ForegroundColor Green; Write-Host "https://www.governmentcontrol.net/" -ForegroundColor Yellow -NoNewline; Write-Host " to access more Microsoft 365 reporting tools. ~~" -ForegroundColor Green `n`n
