...
if($NoMFA.IsPresent)
{
 ...
 if($TenantName -eq "")
 {
  $TenantName = Read-Host "Enter your Tenant Name to complete the process (Example : If your tenant name is 'contoso.com', then enter 'contoso' as a tenant name )  "
 }
...
{ 
  ...
  Write-Host "~~ Check out " -NoNewline -ForegroundColor Green; Write-Host "https://www.governmentcontrol.net/" -ForegroundColor Yellow -NoNewline; 
  Write-Host " for more Microsoft 365 management scripts. ~~" -ForegroundColor Green `n`n
