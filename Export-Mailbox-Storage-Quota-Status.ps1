...
try{
    ...
    {
        # Connect to MgGraph using certificate-based authentication 
    ...
    {
        Connect-MgGraph -scopes "Reports.Read.All","User.Read.All","AuditLog.read.All"  -ErrorAction Stop |Out-Null
    }
}
catch{
...
Function FindMailbox {
    Import-CSV $global:CSVReport | ForEach-Object {
        ...
        # Code section to include last logon time in the report
        $UserPrincipalName=$_.'User Principal Name'
        $UserId=(Invoke-MgGraphRequest -Method GET -Uri  https://graph.microsoft.com/v1.0/users/$UserPrincipalName).id
        $LastSigninTime= (Get-MgUser -Userid $UserId -Property SigninActivity).SignInActivity.LastSignInDateTime
        ...
        if($NearingWarningQuota){
            ...
            if($StorageUsed -ge $NearStorageValue){
                $QuotaStatus = "Storage Used Exceeds $NearingWarningQuota %"
            }                
        }
        else{
            if($global:Flag){
        ...
        if($QuotaStatus -ne ""){ 
            # Export Results
            $ExportResult = @{'User Principal Name' = $_.'User Principal Name'; 'Last Sign-in Time'= $LastSigninTime; 'Storage Used (Bytes)' = $StorageUsed; 'Storage Used' = byteConversion -bytes $storageUsed;'Available Storage' = byteConversion -bytes $AvailableStorage;'Item Count' = $_.'Item Count'; 'Quota Status' = $QuotaStatus; 'Issue Warning Quota (GB)' = [math]::Round($_.'Issue Warning Quota (Byte)' / 1GB, 2); 'Prohibit Send Quota (GB)' = [math]::Round( $_.'Prohibit Send Quota (Byte)' / 1GB, 2); 'Prohibit Send/Receive Quota (GB)' = [math]::Round($_.'Prohibit Send/Receive Quota (Byte)' / 1GB, 2); 'Display Name' = $_.'Display Name';'Recipient Type' = $_.'Recipient Type'; 'Is Mailbox Deleted' = $_.'Is Deleted'}
            $ExportResults = New-Object PSObject -Property $ExportResult
            $ExportResults | Select-object 'Display Name', 'User Principal Name', 'Last Sign-in Time', 'Storage Used', 'Storage Used (Bytes)','Available Storage', 'Issue Warning Quota (GB)', 'Prohibit Send Quota (GB)', 'Prohibit Send/Receive Quota (GB)','Quota Status', 'Recipient Type', 'Item Count', 'Is Mailbox Deleted' | Export-csv -path $global:CSV -NoType -Append -Force  
        }
    }
}
...
Function FetchReport{
    ...
    try{
        Get-MgReportMailboxUsageDetail -period "D7" -OutFile $global:CSVReport -ErrorAction stop
    }
    catch{
      #  Write-Host "Error occurred: $( $_.Exception.Message )" -ForegroundColor Red
...
if (((Test-Path -Path $global:CSV) -ne "True")) {     
    Write-Host "`nNo Mailboxes exceed the storage usage for the given criteria.`n" -ForegroundColor Green
}
else {
    Write-Host "`nThe output file $global:CSV is available in $Location" -ForegroundColor Yellow
    $prompt = New-Object -ComObject wscript.shell    
    $UserInput = $prompt.popup("Do you
