...
2. Allows tracking app operations for a custom date range.
3. The script automatically verifies and installs the Exchange Online PowerShell V3 module (if not installed already) upon confirmation.
4. Enables filtering of app operations from the following categories:
    ...
    -> Credential Changes
    -> Delegation Changes
5. Tracks app operations performed on a specific application.
...
if ($Operations) { $OperationNames = $OperationMap[$Operations] }
else {
    ...
    ForEach ($Result in $Results) {
        ...
        if ($PrintFlag -eq "True") {
            ...
            $ExportResult = @{
                ...
                'Target Application' = $TagetApp
                'Target Type' = $TargetTypes
                'Operation' = $Operation
                'Performed By' = $ActivityBy
            ...
            $ExportResults | Sort 'Event Time' | Select-Object 'Event Time', 'Operation', 'Target Application', 'Performed By', 'Target Type', 'Result Status', 'Modified Properties', 'Event Id', 'More Info' | Export-Csv -Path $OutputCSV -NoTypeInformation -Append
        }
    }
