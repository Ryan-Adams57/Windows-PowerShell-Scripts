<#
=============================================================================================
Name:           Export Azure AD Device Report using MS Graph PowerShell
Description:    This script exports all Azure AD devices to CSV
Version:        1.0
...
(
    ...
    [string[]]$ProfileType,
    [ValidateSet("Azure registered", "Azure joined", "Hybrid joined")]
    [string[]]$JoinType,
...
$OutputCsv = "$Location\AzureADDeviceReport_$($CurrentDate.ToString('yyyy-MMM-dd-ddd hh-mm-ss tt')).csv"
$Report=""
$PrintedLogs=0
...
Get-MgDevice -All | ForEach-Object {
    ...
    if ($_.TrustType -eq "Workplace") { $TrustType = "Azure registered" }
    elseif ($_.TrustType -eq "AzureAd") { $TrustType = "Azure joined" }
    elseif ($_.TrustType -eq "ServerAd") { $TrustType = "Hybrid joined" }
...
Write-Host "~~ Check out " -NoNewline -ForegroundColor Green; Write-Host "GitHub: https://github.com/Ryan-Adams57" -ForegroundColor Yellow -NoNewline; Write-Host " for more projects. ~~" -ForegroundColor Green `n
