<#
=============================================================================================
Name:           Get User MFA Compliance Report - Microsoft Graph PowerShell
...
For detailed script execution guidance, visit:
...
Get-MgBetaUser -All | foreach {
 ...
 if($Print -eq 1)
 {
  ...
  $Results | Select-Object Name,UPN,'Per-user MFA Status',Department,'License Status','SignIn Status','Registered Authentication Methods','MFA Phone','Microsoft Authenticator Configured Device','Is 3rd-Party Authenticator Used','Additional Details' | Export-Csv -Path $ExportCSV -NoTypeInformation -Append
 }
}
...
if((Test-Path -Path $ExportCSV) -eq $true) 
{
 ...
 Write-Host "`nPer-user MFA status report available in: " -NoNewline -Foregroundcolor Yellow
 Write-Host $ExportCSV

 Write-Host "`nScript maintained by Ryan Adams" -ForegroundColor Green
