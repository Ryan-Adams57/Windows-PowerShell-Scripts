...
5. Automatically installs the Microsoft Graph PowerShell module (if not installed already) upon confirmation.
6. Supports MFA-enabled accounts.
7. Can be executed with certificate-based authentication (CBA). 
8. Scheduler-friendly.
...
if ($MsGraphModule -eq $null) {
    Write-Host "`nImportant: Microsoft Graph module is unavailable. It is mandatory to have this module installed to run the script successfully." 
    ...
    if ($confirm -match "[yY]") {
        ...
        Write-Host "Exiting. `nNote: Microsoft Graph module must be available to run this script." -ForegroundColor Red
        Exit
    }
} 
...
if ($Script:ProcessedCount -ne 0) {
    ...
    Write-Host "`nEmail has been sent successfully." -ForegroundColor Yellow
    
    if ((Test-Path -Path $CSVFilePath) -eq "True") {
        Write-Host "`nThe output file saved at: " -NoNewline -ForegroundColor Yellow
        Write-Host $CSVFilePath
    }
} else{
    Write-Host "`nNo app(s) found with secrets or certificates expiring in the next $SoonToExpireInDays days." -ForegroundColor Yellow
}
...
Write-Host "~~ Check out " -NoNewline -ForegroundColor Green; Write-Host "https://github.com/Ryan-Adams57" -ForegroundColor Yellow -NoNewline; Write-Host " or https://gitlab.com/Ryan-Adams57 for additional scripts. ~~" -ForegroundColor Green
