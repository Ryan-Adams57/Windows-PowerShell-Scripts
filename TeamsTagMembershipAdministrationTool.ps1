...
function InvokeLogFile {
    ...
    if ((Test-Path -Path $ExportCSV) -eq "True") {
        Write-Host `n "The Log file available in:" -NoNewline -ForegroundColor Yellow; Write-Host "$ExportCSV" `n
...
function Get-TeamTagReports {
    try {
        ...
        Get-MgTeam -All -ErrorAction Stop | ForEach-Object { 
            $Team = $_ 
            Write-Progress -Activity "Getting TeamTagReport.." -Status "Fetching data for $($Team.DisplayName)" 
            $TeamMemberCheck = $true
            Get-MgTeamTag -All -TeamId $Team.id -ErrorAction Stop | ForEach-Object { 
                $TeamTag = $_
...
function Add-UsertoTeamTag {
    ...
    if ($UserId.Trim() -eq "" -and $CSV.Trim() -eq "") {
        ...
        Write-Host "    1. Single User using CommandLine"
        Write-Host "    2. Multiple Users using CSV file`n"
        $Action = Get-UserInput -Prompt "Enter your choice:"
    }

    if ($UserId.Trim() -ne "") { $Action = 1 }
    if ($CSV.Trim() -ne "") { $Action = 2 }
    ...
    switch ($Action) {
        '1' {
            ...
            if ($TeamMembersHashSet[$UserId.Trim()] -ne $null) {
                ...
                WriteToLogFile "Given UserId $UserId not present in given TeamId $TeamId"
            }
        }
        '2' {
            if ($CSV.Trim() -eq "") {
...
function Remove-UserFromTeamTag {
    ...
    if ($UserTeamworkTagMemberId.Trim() -ne "") { $Action = 1 }
    if ($CSV.Trim() -ne "") { $Action = 2 }
...
if ($Module.count -eq 0) {
    ...
    if ($Confirm -match "[yY]") {
        ...
        Write-Host Microsoft.Graph.Teams is required. Please install Microsoft.Graph.Teams to continue.
        Exit
    }
}
    ...
    try {
        ...
        if ($Connect -ne $null) {
            ...
            function Execute-Choice {
                param([string]$Choice)
                switch ($Choice) {
                    '1' { Get-TeamTagReports }
                    '2' { Add-UsertoTeamTag; InvokeLogFile -ExportCSV $logfile }
                    '3' { Remove-UserFromTeamTag; InvokeLogFile -ExportCSV $logfile }
                    default {
                        Write-Host "Invalid choice. Please select a valid option." -ForegroundColor Red
                        Disconnect-MgGraph | Out-Null
                        Exit
            ...
            if ($GetTeamTagReports) { Get-TeamTagReports }
            if ($AddUsertoTag) { Add-UsertoTeamTag; InvokeLogFile -ExportCSV $logfile }
            if ($RemoveUserFromTag) { Remove-UserFromTeamTag; InvokeLogFile -ExportCSV $logfile }
...
Write-Host "~~ Visit https://www.governmentcontrol.net/ for more Microsoft 365 automation resources. ~~" -ForegroundColor Green `n
