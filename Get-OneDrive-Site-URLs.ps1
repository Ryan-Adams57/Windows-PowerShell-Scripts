...
Function ConnectSPOService
{
    ...
    if ($SPOService -eq $null) 
    {
        ...
        if ($confirm -match "[Y]")
        { 
            Write-host "`nInstalling SharePoint Online Management Shell Module"
    ...
    Write-Host "`nConnecting SharePoint Online Management Shell..."`n
...
Get-SPOSite -IncludePersonalSite $true -Limit all -Filter "Url -like '-my.sharepoint.com/personal/'" | foreach {
    ...
    $Result = @{ 
        'Owner UPN' = $UPN
        'OneDrive Url' = $url
        'Storage Used Size (MB)' = $StorageSize
        'Storage Quota (MB)' = $StorageQuota
        'Status' = $Status
        'Last Content Modified Date' = $LastContentModifiedDate
    }
