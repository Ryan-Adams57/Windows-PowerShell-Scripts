...
Function Installation-Module{
    ...
    If($Module -eq $null){
        ...
        $Confirm = Read-Host "Are you sure you want to install the module? [Yy] Yes [Nn] No"
        If($Confirm -match "[yY]") { 
...
$ReportOutput = "$(Get-Location)\Expired_SPO_Anonymous_Links_$timestamp.csv"
$Global:ItemCount = 0

If($TenantName -eq "" -and $ImportCsv -eq "")
{
...
if((Test-Path -Path $ReportOutput) -eq "True") 
{
    ...
    Write-Host "~~ Check out " -NoNewline -ForegroundColor Green; Write-Host "GitHub - https://github.com/Ryan-Adams57" -ForegroundColor Yellow -NoNewline; Write-Host " for more information. ~~" -ForegroundColor Green `n`n
