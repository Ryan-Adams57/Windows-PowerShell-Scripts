...
Connect_Exo
Write-Host "Note: Ensure that you have assigned Mailbox Import Export role." -ForegroundColor Cyan
...
if ($UPN -ne "") {
    if (($Days -eq -1)) {
       ...
       try {
            ...
            Write-Host "Error occurred, Please go through your inputs and try again" -ForegroundColor Red
        }
    }
    else {
        try {
...
elseif ($CSV -ne "") {
    if ($Days -eq -1) {
        ...
        Import-Csv $CSV | ForEach-Object {
            ...
            try {
                ...
                WriteToLogFile "Error Occurred while deleting mail from $UPN mailbox. Please check the inputs and try again."
            }
        }
    }
    else {
    ...
    try {
        ...
        Write-Host "Error occurred, Please check whether you have necessary permissions and go through your inputs" -ForegroundColor Red
    }
}

if ((Test-Path -Path $global:logfile) -eq "True") {
    if ((Test-Path -Path $global:ExportCSVFileName) -eq "True") {
        Write-Host "Deleted email size report available in `"$global:ExportCSVFileName`"" -ForegroundColor Green 
        Write-Host "The Logfile is available in $global:logfile."
