...
4. The script uses CQDPowerShell and installs it (if not installed already) upon your confirmation. 
5. The script is scheduler friendly. 
6. The script sets for 28 days by default as EUII data is only available for 28 days. 
...
# Retrieving data for past 28 days
$EndDate = Get-Date
    ...
    switch($report) {
        ...
        "MeetingInfo" {
            ...
            Get-CQDData -StartDate $StartDate -EndDate $EndDate -Dimensions $dimensions -Measures $measures -OutputType DataTable | ForEach-Object {
                ...
                if($IsTeams)
                {
                    ...
                    $ExportResult = @{
                        "Date" = $date;
                        "Meeting/Call Id" = $ConferenceId;
                        "Organizer UPN" = $OrganizerUPN;
                        "UPN of first end of stream" = $FirstUPN;
                        "UPN of second end of stream" = $SecondUPN;
                        "Network Transport Type" = $Transport;
                        "Session Type" = $SessionType;
                        "Avg Network Score (0-10)" = $AvgNetworkScore;
                        "Total Stream Duration" = $AttendedDuration;
        ...
        "NetworkQuality" {
            ...
            Get-CQDData -StartDate $StartDate -EndDate $EndDate -Dimensions $dimensions -Measures $measures -OutputType DataTable | ForEach-Object {
                ...
                if($data.'Is Teams')
                {
                    $SessionType = if($data.'Session Type' -eq "P2P") {"Peer-to-Peer"} elseif($data.'Session Type' -eq "Conf") {"Meeting"} else {"-"}

                    $ExportResult = @{
                        "Date" = $data.'Date';
                        "Meeting/Call Id" = $data.'Conference Id';
                        "UPN of first end of stream" = $data.'First UPN';
                        "UPN of second end of stream" = $data.'Second UPN';
                        "Session Type" = $SessionType;
                        "ISP for first end of stream" = $data.'First ASN ISP Name';
                        "ISP for second end of stream" = $data.'Second ASN ISP Name';
                        "Average Jitter (ms)" = $data.'Avg Jitter';
                        "Average Packet Loss Rate (0-1)" = $data.'Avg Packet Loss Rate';
                        "Average Round Trip (ms)" = $data.'Avg Round Trip'
                    }

                    New-Object PSObject -Property $ExportResult | 
                    Select-Object "Date","Meeting/Call Id","UPN of first end of stream","UPN of second end of stream","Session Type","ISP for first end of stream","ISP for second end of stream","Average Jitter (ms)","Average Packet Loss Rate (0-1)","Average Round Trip (ms)" | 
                    Export-Csv -Path $OutfilePath -NoTypeInformation -Append
                }
            }
            break
        }
        ...
        "AudioHealth" {
            ...
            Get-CQDData -StartDate $StartDate -EndDate $EndDate -Dimensions $dimensions -Measures $measures -OutputType DataTable | ForEach-Object {
                if($_.'Is Teams')
                {
                    $ExportResult = @{
                        "Date" = $_.'Date';
                        "Meeting/Call Id" = $_.'Conference Id';
                        "Good Call Stream" = $_.'Audio Good Call Stream Count';
                        "Poor Call Stream" = $_.'Audio Poor Call Stream Count';
                        "Unclassified Call Stream" = $_.'Audio Unclassified Call Stream Count';
                        "Poor Percentage" = if([string]::IsNullOrEmpty($_.'Audio Poor Percentage')){"-"} else {$_.'Audio Poor Percentage'};
                        "Total Audio Stream Count" = $_.'Audio Stream Count'
                    }

                    New-Object PSObject -Property $ExportResult |
                    Select-Object "Date","Meeting/Call Id","Good Call Stream","Poor Call Stream","Unclassified Call Stream","Poor Percentage","Total Audio Stream Count" |
                    Export-Csv -Path $OutfilePath -NoTypeInformation -Append
                }
            }
            break
            ...
            Get-CQDData -StartDate $StartDate -EndDate $EndDate -Dimensions $dimensions -Measures $measures -OutputType DataTable | ForEach-Object {
                if($_.'Is Teams')
                {
                    $ExportResult = @{
                        "Date" = $_.'Date';
                        "Meeting/Call Id" = $_.'Conference Id';
                        "Good Call Stream" = $_.'Video Good Stream Count';
                        "Poor Call Stream" = $_.'Video Poor Stream Count';
                        "Unclassified Call Stream" = $_.'Video Unclassified Stream Count';
                        "Poor Percentage" = if([string]::IsNullOrEmpty($_.'Video Poor Percentage')){"-"} else {$_.'Video Poor Percentage'};
                        "Total Video Stream Count" = $_.'Video Stream Count'
                    }

                    New-Object PSObject -Property $ExportResult |
                    Select-Object "Date","Meeting/Call Id","Good Call Stream","Poor Call Stream","Unclassified Call Stream","Poor Percentage","Total Video Stream Count" |
                    Export-Csv -Path $OutfilePath -NoTypeInformation -Append
                }
            }
            break
            ...
            Get-CQDData -StartDate $StartDate -EndDate $EndDate -Dimensions $dimensions -Measures $measures -OutputType DataTable | ForEach-Object {
                if($_.'Is Teams')
                {
                    $ExportResult = @{
                        "Date" = $_.'Date';
                        "Meeting/Call Id" = $_.'Conference Id';
                        "Good Call Stream" = $_.'VBSS Good Stream Count';
                        "Poor Call Stream" = $_.'VBSS Poor Stream Count';
                        "Unclassified Call Stream" = $_.'VBSS Unclassified Stream Count';
                        "Poor Percentage" = if([string]::IsNullOrEmpty($_.'VBSS Poor Percentage')){"-"} else {$_.'VBSS Poor Percentage'};
                        "Total ScreenSharing Stream Count" = $_.'VBSS Stream Count'
                    }

                    New-Object PSObject -Property $ExportResult |
                    Select-Object "Date","Meeting/Call Id","Good Call Stream","Poor Call Stream","Unclassified Call Stream","Poor Percentage","Total ScreenSharing Stream Count" |
                    Export-Csv -Path $OutfilePath -NoTypeInformation -Append
                }
            }
            break
            ...
            Get-CQDData -StartDate $StartDate -EndDate $EndDate -Dimensions $dimensions -Measures $measures -OutputType DataTable | ForEach-Object { 
                if($_.'Is Teams')
                {
                    $ExportResult = @{
                        "Date" = $_.'Date';
                        "Meeting/Call Id" = $_.'Conference Id';
                        "Device used in first end of stream" = $_.'First Compute Device Name';
                        "Device used in second end of stream" = $_.'Second Compute Device Name';
                        "Total Audio Stream Duration (Minutes)" = $_.'Total Audio Stream Duration (Minutes)';
                        "First end user device used count" = $_.'First User Count';
                        "Second end user device used count" = $_.'Second User Count'
                    }

                    New-Object PSObject -Property $ExportResult |
                    Select-Object "Date","Meeting/Call Id","Device used in first end of stream","Device used in second end of stream","Total Audio Stream Duration (Minutes)","First end user device used count","Second end user device used count" |
                    Export-Csv -Path $OutfilePath -NoTypeInformation -Append
                }
            }      
            break
...
Write-Host "`nThe location of output file is : $OutputPath`n" -ForegroundColor Yellow
